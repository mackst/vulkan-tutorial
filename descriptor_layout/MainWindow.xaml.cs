...
namespace descriptor_layout
{
        ...
        public float[] ToArray()
        {
            ...
            foreach(var mat in matrices)
            {
                for(var i = 1; i < 5; i++)
                {
                    for(var y = 1; y < 5; y++)
                    {
                        var info = mtype.GetProperty(string.Format("M{0}{1}", i, y));
                        outArray.Add((float)info.GetValue(mat));
                    }
                }
            }
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            DescriptorSetLayoutBinding[] uboLayoutBindings =
            {
        ...
        public void CopyBuffer(Vulkan.Buffer srcBuffer, Vulkan.Buffer dstBuffer, DeviceSize size)
        {
            ...
            device.FreeCommandBuffer(commandPool, cmdBuffer);
        }
