...
class HelloTriangleApplication(QtGui.QWindow):
    ...
    def __createVertexBuffer(self):
        ...
        self.__vertexBuffer = vkCreateBuffer(self.__device, bufferInfo, None)

        memRequirements = vkGetBufferMemoryRequirements(self.__device, self.__vertexBuffer)

        allocInfo = VkMemoryAllocateInfo(
            sType=VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
            allocationSize=memRequirements.size,
            memoryTypeIndex=self.__findMemoryType(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT)
        )

        self.__vertexBufferMemory = vkAllocateMemory(self.__device, allocInfo, None)

        vkBindBufferMemory(self.__device, self.__vertexBuffer, self.__vertexBufferMemory, 0)

        vertex_ptr = ffi.cast('float *', self.vertices.ctypes.data)
        data = vkMapMemory(self.__device, self.__vertexBufferMemory, 0, bufferInfo.size, 0)
        ffi.memmove(data, vertex_ptr, bufferInfo.size)
        vkUnmapMemory(self.__device, self.__vertexBufferMemory)
